	...
	<script type="text/javascript">
		$(function () {
			...
			// 생성한 마커의 정보를 저장할 공간 (마커가 여러개인 경우)
				...
				// 기존에 등록했던 주소(좌표)
				var regionX = $("[name=eventRegionX]").val();
				var regionY = $("[name=eventRegionY]").val();
...
<div class="container w-800">
	...
	<form action="edit" method="post" autocomplete="off" class="check-form" enctype="multipart/form-data" >
	<input type="hidden" name="eventNo" value="${eventDto.eventNo}">
			<div class="cell">
					...
					<div class="cell">
						...
						<div class="flex-fill ms-20">
							...
							<div class="cell flex-box w-100">
								<input type="hidden" name="eventRegionX" value="${eventDto.eventRegionX}" readonly>
								<input type="hidden" name="eventRegionY" value="${eventDto.eventRegionY}" readonly>
							</div>
						</div>	
				</div>
				...
				<div class="cell">
					<label>내용</label>
					<textarea class="summernote-editor" name="eventContent" required>${eventDto.eventContent}</textarea>
					<div class="success-feedback">올바른 입력입니다</div>
                    <div class="fail-feedback">필수 항목입니다</div>
				</div>
