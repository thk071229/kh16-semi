	...
	<div class="container">
		<div class="cell">
			...
			<c:if test = "${pageVO.firstBlock == false}">
				<a href = "home?page=1${pageVO.searchParams}${pageVO.parentParamsToString}">&lt;&lt;</a>
				<%-- &{pageVO.blockStart > 1} = 첫 블록이 아니면 --%>
				<%-- 가독성을 위해 pageVO에 boolean 메소드 생성 --%>
				<%-- EL에서는 ==false 대신 not 사용 가능하나 선택사항 --%>
				<a href = "home?page=${pageVO.prevPage}${pageVO.searchParams}${pageVO.parentParamsToString}">&lt;</a>
			...
			<%-- finish 값이 totalPages보다 작으면 finish(원래의 end값) 반환, 크면 totalPages(마지막 페이지 값을 end로) 반환 --%>
				<c:choose>
					...
					<a href = "home?page=${i}${pageVO.searchParams}${pageVO.parentParamsToString}">${i}</a>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			...
			<a href = "home?page=${pageVO.nextPage}${pageVO.searchParams}${pageVO.parentParamsToString}">&gt;</a>
			<%-- ${finish + 1} = 다음 페이지 --%>
			<%-- 인덱스의 끝 경계인 finish(ex:20(11-20블럭))에서 +1을 해줘야 다음 페이지(ex:21-30블럭))로 넘어감--%>
			<a href = "home?page=${pageVO.totalPage}${pageVO.searchParams}${pageVO.parentParamsToString}">&gt;&gt;</a>
			</c:if>
			</div>
		</div>
	</div>
