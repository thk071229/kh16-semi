	...
	<div class="container">
		<div class="cell">
			...
			<c:if test = "${pageVO.firstBlock == false}">
				<a href = "category?page=1${pageVO.searchParams}${pageVO.parentParamsToString}&categoryNo=${categoryDto.categoryNo}">&lt;&lt;</a>
				<%-- &{pageVO.blockStart > 1} = 첫 블록이 아니면 --%>
				<%-- 가독성을 위해 pageVO에 boolean 메소드 생성 --%>
				<%-- EL에서는 ==false 대신 not 사용 가능하나 선택사항 --%>
				<a href = "search?page=${pageVO.prevPage}${pageVO.searchParams}${pageVO.parentParamsToString}&categoryNo=${categoryDto.categoryNo}">&lt;</a>
				...
				<c:forEach var="i" begin = "${pageVO.blockStart}" end = "${pageVO.blockFinish}" step = "1">
					...
					<c:choose>
						...
						<%-- 현재 페이지가 아닐 경우 클릭하여 이동할 수 있도록 처리 --%>
							<a href = "category?page=${i}${pageVO.searchParams}${pageVO.parentParamsToString}&categoryNo=${categoryDto.categoryNo}">${i}</a>
						</c:otherwise>
					</c:choose>
				</c:forEach>
					...
					<%-- 현재 페이지의 끝 값(finish) = 50 > 전체 페이지 수 (totalPages = 45) 이므로 다음 표시 x --%>
						<a href = "category?page=${pageVO.nextPage}${pageVO.searchParams}${pageVO.parentParamsToString}&categoryNo=${categoryDto.categoryNo}">&gt;</a>
						<%-- ${finish + 1} = 다음 페이지 --%>
						<%-- 인덱스의 끝 경계인 finish(ex:20(11-20블럭))에서 +1을 해줘야 다음 페이지(ex:21-30블럭))로 넘어감--%>
						<a href = "category?page=${pageVO.totalPage}${pageVO.searchParams}${pageVO.parentParamsToString}&categoryNo=${categoryDto.categoryNo}">&gt;&gt;</a>
				</c:if>
			</div>
		</div>
</div>
